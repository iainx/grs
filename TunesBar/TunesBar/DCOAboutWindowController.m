...
- (void)windowDidLoad {
    ...
    [self.visitWebsiteButton sizeToFit];
    
    // Realign the visit website button
    NSRect acknowledgementsFrame = self.acknowledgmentsButton.frame;
    NSRect visitFrame = self.visitWebsiteButton.frame;
    
    self.visitWebsiteButton.frame = NSMakeRect(acknowledgementsFrame.origin.x - 20 - visitFrame.size.width,
                                               visitFrame.origin.y,
                                               visitFrame.size.width, visitFrame.size.height);
...
- (void)setAcknowledgmentsPath:(NSString *)acknowledgmentsPath {
    ...
    if(!acknowledgmentsPath) {
        ...
        // Realign the website button
        NSRect frame = self.visitWebsiteButton.frame;
        NSRect infoViewBounds = self.infoView.bounds;
        [self.visitWebsiteButton setFrame:NSMakeRect(NSMaxX(infoViewBounds) - frame.size.width - 20.0,
...
- (IBAction)showAcknowledgments:(id)sender {
    ...
    if(self.acknowledgmentsPath) {
        ...
        NSAttributedString *acknowledgmentsString = [[NSAttributedString alloc] initWithPath:self.acknowledgmentsPath
                                                                          documentAttributes:nil];
        [self.creditsTextView.textStorage setAttributedString:acknowledgmentsString];
    } else {
